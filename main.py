...
return “Log recorded”
def fn_3_math(e):
try: return eval(e, {”**builtins**”: None}, {})
...
# Catches every POST not matched by a named route above.

# FastAPI evaluates routes in order, so /deploy is still hit when called directly.
...
```
# Pull a trigger string from common keys, or fall back to the raw body
trigger = (
    body.get("input")
    or body.get("message")
    or body.get("text")
