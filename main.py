...
stale = [k for k, v in RATE_STORE.items() if v and now - v[-1] > RATE_WINDOW * for k in stale:
del RATE_STORE[k]
...
return JSONResponse({"error": "Internal server error", "detail": str(exc)}, statu
...
try:
# FIX BUG-1: __import__ was mangled to **import** by markdown renderer
__import__(name)
except ImportError:
...
"model": "compound-beta",
"messages": [
{"role": "system", "content": "You are a Python code generator. Outpu
...
PRMPTS = [
"Critically analyze the current state. What is missing to reach AI Engineer status? You M
"Generate a hypothesis for a better autonomous pattern. You MUST call tool='log' with arg
"Identify the single most critical failure point in the previous step. You MUST call tool
"You MUST call tool='mut' with args={'p': '<one sentence ruleset>'}. Keep p under 100 cha
"Log a one-sentence final verification summary. You MUST call tool='log' with args={'m':
"MANDATORY FINAL STEP – no other tool is valid here. You MUST call tool='align' with args
]
# ─── GROQ RATE LIMITING ───────────────────────────────────────────────────────
...
logger.warning(f"[Groq throttle] RPD cap – waiting {wait:.0f}s (~{wait/3600:.
...
logger.warning(f"[Groq throttle] TPM cap ({tokens_used} used) – waiting {wait
await asyncio.sleep(wait)
ts = time.time()
...
"https://api.groq.com/openai/v1/chat/completions",
headers={"Authorization": f"Bearer {K}", "Content-Type": "application/jso
json={
# FIX BUG-4: valid Groq model name
"model": "compound-beta",
"messages": [
...
data = {"tool": "mut", "args": {"p": safe}, "thought": "extracted from ma
else:
logger.error(f"[Loop] Step {i}: failed to parse LLM response: {e} | raw={
continue
...
t = "mut"
a = {"p": f"Goal: AI Engineer. Full analysis required. Context: {ctx[-200:].repla
if i == 5 and t != "align":
...
ctx += f"\n[Step {i}] Commit blocked (runs at end) | Reasoning: {data.get('though
continue
if t in TOOLS:
try:
res = TOOLS[t](**a)
