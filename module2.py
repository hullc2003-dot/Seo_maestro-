...
def fn_7_mut(new_rule: str) -> None:
    ...
    asyncio.create_task(fn_signal_ui("Operational rules mutated"))

async def fn_signal_ui(msg: str):
...
async def fn_align_with_spec(**kwargs):
    try:
        ...
        gap_prompt = f"spec: {spec_snippet} | code tail: {code_snippet} | ONE sentence: top missing capability?"
...
async def fn_read_github(path):
    # For testing: in production, replace with real GH API GET call
